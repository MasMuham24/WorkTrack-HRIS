...
class AttendanceController extends Controller
{
    ...
    public function index()
    {
        ...
        $attendances = $this->attendanceService->getAttendanceHistory();
        $offices = $this->attendanceService->getOffices();

        return view(
            'employee.attendance.index',
            compact('todayAttendance', 'attendances', 'offices')
        );
    }
    ...
    public function checkIn(Request $request): JsonResponse|RedirectResponse
    {
        ...
        try {
            ...
            return $request->ajax() ? response()->json(['message' => $msg]) : redirect()->route('employee.attendance.index')->with('success', $msg);
        } catch (\RuntimeException $e) {
            return $request->ajax() ? response()->json(['message' => $e->getMessage()], 422) : back()->with('error', $e->getMessage());
        }
    }
